...
class SchNet(torch.nn.Module):
    ...
    def forward(self, batch_data, assistant_outs: list=None) -> Tensor:
        z, pos, batch = batch_data.z, batch_data.pos, batch_data.batch
        if assistant_outs is not None:
            assistant_emb_out, assistant_inter_outs, assistant_aggr_out = assistant_outs[0], assistant_outs[1:1+self.num_interactions], assistant_outs[-1]
        else:
            assistant_emb_out, assistant_inter_outs, assistant_aggr_out = None, None, None
        ...
        h = self.embedding(z)
        if assistant_emb_out is not None:
            h = h + assistant_emb_out
        
        edge_index, edge_weight = self.interaction_graph(pos, batch)
        edge_attr = self.distance_expansion(edge_weight)

        for i, interaction in enumerate(self.interactions):
            if assistant_inter_outs is None:
                h = h + interaction(h, edge_index, edge_weight, edge_attr)
            else:
                h = h + interaction(h, edge_index, edge_weight, edge_attr) + assistant_inter_outs[i]
            

        h = self.lin1(h)
        h = self.act(h)
        if assistant_aggr_out is not None:
            h = h + assistant_aggr_out
        h = self.lin2(h)
