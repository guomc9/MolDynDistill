...
class MD17(InMemoryDataset):
    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        for raw_path, processed_path in it:
            ...
            for i in range(pos.size(0)):
                data = Data(z=z, pos=pos[i], energy=energy[i], y=energy[i], force=force[i])
    ...
    # def split(self, train_ratio, valid_ratio, seed):
    #     data_size = len(self)
    #     train_size = data_size * train_ratio
    #     valid_size = data_size * valid_ratio
    #     ids = shuffle(range(data_size), random_state=seed)
    #     train_idx, val_idx, test_idx = torch.tensor(ids[:train_size]), torch.tensor(ids[train_size:train_size + valid_size]), torch.tensor(ids[train_size + valid_size:])
    #     train_dataset, valid_dataset, test_dataset = self[train_idx], self[val_idx], self[test_idx]
